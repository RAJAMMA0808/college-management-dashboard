        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <select value={role} onChange={e => setRole(e.target.value as Role)} className={inputClass('role')} onFocus={() => setFocused('role')} onBlur={() => setFocused(null)}>
              {/* FIX: Using Object.entries for type-safe iteration over roles. This resolves issues where Object.values on an enum can lead to type inference problems. */}
          ...
          {role !== Role.CHAIRMAN && (
            <div>
              ...
              <select value={college} onChange={e => setCollege(e.target.value as College)} className={inputClass('college')} onFocus={() => setFocused('college')} onBlur={() => setFocused(null)}>
                {/* FIX: Using Object.entries for type-safe iteration over colleges. This resolves issues where Object.values on an enum can lead to type inference problems. */}
